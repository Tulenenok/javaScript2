...
Vue.component('basketItem', {
    ...
    template: `<div v-if="item.quantity>0 "class = 'basket-item' :data-title = "item.product_name" :data-price = "item.price" :data-id_product = "item.id_product" :data-id = "item.id_product" :data-image = "item.image" data-quantity = "item.quantity">
                    ...
                    <button class = 'basket-button' @click = "$root.removeProduct($event)" :data-title = "item.product_name" :data-price = "item.price" :data-id = "item.id_product" :data-id_product = "item.id_product" :data-image = "imgBasket" data-quantity = "item.quantity">-</button>
                </div>`
})
